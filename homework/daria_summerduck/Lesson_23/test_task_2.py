...
def test_fill_form(
    ...
    month = driver.find_element(
        By.XPATH,
        f'//select[@class="react-datepicker__month-select"]//option'
        f'[text()="{month_of_birth_value}"]',
    )
    ...
    day = driver.find_element(
        By.XPATH, f'//div[contains(@aria-label,"{month_of_birth_value} {day_value}")]'
    )
    day.click()
    ...
    expected_modal_content = f"""
        Student Name {first_name_value} {last_name_value}
        ...
        Picture
        Address {current_address_value}
        State and City {state_and_city_value}
    """
    print(expected_modal_content)
    # assert (
    #     modal_content == expected_modal_content
    # ), "Expected modal content is not equal to the actual one"
